...
class CustomSubMaker:
    """
    A Custom SubMaker to maintain compatibility with older edge_tts integrations.
    Newer edge_tts versions removed `.offset` and `.subs` attributes.
    """
    def __init__(self):
        self.offset = []
        self.subs = []
        self.cues = []  # Added to pass hasattr(sub_maker, "cues") checks
...
def azure_tts_v1(
    ...
    for i in range(3):
        try:
            ...
            async def _do() -> CustomSubMaker:
                communicate = edge_tts.Communicate(text, voice_name, rate=rate_str)
                # Use CustomSubMaker instead of edge_tts.SubMaker
                ...
                with open(voice_file, "wb") as file:
                    async for chunk in communicate.stream():
                        if chunk["type"] == "audio":
                            ...
                            # Handle SentenceBoundary events (newer edge-tts versions)
                            sub_maker.create_sub(
                                (chunk["offset"], chunk["duration"]), chunk["text"]
                            )
                return sub_maker
            ...
            if not sub_maker or not hasattr(sub_maker, 'offset'):
                logger.warning("failed, sub_maker is None or has no offset attribute")
                continue
...
def create_subtitle(sub_maker, text: str, subtitle_file: str):
    """
    优化字幕文件
    1. 将字幕文件按照标点符号分割成多行
    2. 逐行匹配字幕文件中的文本
    ...
    def formatter(idx: int, start_time: float, end_time: float, sub_text: str) -> str:
        ...
        跑步是一项简单易行的运动
        """
        # Proper conversion from 100-nanoseconds to SRT time format
        def format_time(time_100ns):
            seconds = time_100ns / 10000000.0
        ...
        start_t = format_time(start_time)
        end_t = format_time(end_time)
        return f"{idx}\n{start_t} --> {end_t}\n{sub_text}\n"
    ...
    try:
        # Handle both CustomSubMaker and edge_tts.SubMaker
...
def _get_audio_duration_from_submaker(sub_maker: Union[submaker.SubMaker, CustomSubMaker]):
    """
    获取音频时长
    """
...
def get_audio_duration(target: Union[str, submaker.SubMaker, CustomSubMaker]) -> float:
    """
    Get the duration of the audio
    If it's a SubMaker object or a CustomSubMaker object, get the duration from SubMaker
    If it's an MP3 file, get the duration from the MP3 file
    """
    if isinstance(target, submaker.SubMaker):
        return _get_audio_duration_from_submaker(target)
    elif isinstance(target, CustomSubMaker):
        return _get_audio_duration_from_submaker(target)
